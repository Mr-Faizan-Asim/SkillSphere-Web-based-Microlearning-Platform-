...
router.get('/mentor-applications', auth,  async (req, res) => {
  const pending = await User.find({ role: 'mentor', 'mentorProfile.verified': false });
  res.json(pending);
});

// Approve mentor
router.patch('/mentor-applications/:id/approve', auth,  async (req, res) => {
...
// Analytics
router.get('/analytics', auth,  adminController.analytics);
